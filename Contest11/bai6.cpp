...
public:
    ...
    Node *buildTree(vector<int> in, vector<int> level, int lvStart, int lvEnd);
    //! Some special method
...
int lvIndex = 0;
Node *Tree::buildTree(vector<int> in, vector<int> level, int lvStart, int lvEnd)
{
    ...
    traversal using lvIndex  
    and increment lvIndex */
    Node *tNode = create(level[lvIndex]);
    if (lvIndex == 0)
    {
        this->rt = tNode;
    }
    ...
    right subtress */
    tNode->left = buildTree(in, level, lvStart, inIndex - 1);
    tNode->right = buildTree(in, level, inIndex + 1, lvEnd);
